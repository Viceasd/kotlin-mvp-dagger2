...
class ExampleUnitTest {
    ...
    fun paseoJson(){
        ...
        val gsonContent1 = gson.toJsonTree(gsonContent,  Indicador::class.java)
        val jsonObject = gsonContent1 as JsonObject

        val attributes = HashMap<String, String>()
        val entrySet =  jsonObject.get("dolar").asJsonObject.entrySet()
        for ((key) in entrySet) {
            attributes.put(key, (jsonObject.get("dolar").asJsonObject).get(key).asString)
        }


        // aqui pasarlo a json y tomar el valor dolar
        var listFragment : ListFragment = ListFragment()

        listFragment.prepareListDetalle(gsonContent,"dolar")
    }
}
